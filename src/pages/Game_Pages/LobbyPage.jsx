...
import { Clock, Play, Users, Timer, Eye } from "lucide-react";
...
function LobbyPage() {
  ...
  // ── Listen for opponent lock/unlock and game-session-started ──
  useEffect(() => {
    ...
    const handleGameStarted = ({
      ...
      navigate("/Game", {
        state: {
          ...
          isHost: role === "host",
          secretNumber,
          // Pass initial turn state so GameSessionPage doesn't start blind
  ...
  const handleReady = () => {
    if (!secretNumber || isNaN(secretNumber)) {
      toast.error("Enter a valid secret number first.");
      return;
    }
    setIsReady(true);
    if (opponent?._id) {
   ...
   * (same as guest), which eliminates the race condition.
   */
  const handleStartGame = () => {
    if (!secretNumber || isNaN(secretNumber)) {
      toast.error("Choose a secret number before starting.");
      return;
    }
    if (!isReady) {
    ...
    socket.emit("start-game-session", {
      hostId: currentUser?._id,       // host is always the socket.userId (enforced server-side)
      guestId: opponent?._id,
      secretNumber,
      guessingTimer: Number(guessingTimer) || 3,
      responseTimer: Number(responseTimer) || 3,
    });
    // ⚠️ No navigate() here — wait for game-session-started from server
  };
  ...
  const players = [
    ...
    opponent
      ? {
          ...
          isHost: !isHost,
          ready: opponentReady,
          subtitle: opponentReady ? "Locked in" : "Choosing a number…",
        }
      : null,
  ].filter(Boolean);
      ...
      <div className="flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex-1 px-4 py-4 flex flex-col gap-2">
          {players.map((player, idx) => (
            <div
              key={player.id}
              className="flex items-center gap-4 rounded-xl border border-gray-100 px-4 py-3 hover:bg-gray-50 transition"
            >
